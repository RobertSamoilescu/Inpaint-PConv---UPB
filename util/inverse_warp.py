...
#     aux = aux.repeat(batch_size, 1, 1)                    # [B, 1, 4]
#     aux_extrinsics = torch.cat((extrinsics, aux), dim=1)  # [B, 4, 4]

#     # compute real world position
#     # world_coords = aux_extrinsics.inverse() @ cam_coords
...
#     # pose_mat = pose_mat.repeat(batch_size, 1, 1)        # [B, 3, 4]
#     # pose_mat = torch.cat((pose_mat, aux), dim=1)        # [B, 4, 4]

#     # projection matrix
#     # proj_matrix = intrinsics @ extrinsics @ pose_mat
...
#     #     img, dst_pixel_coords, padding_mode=padding_mode)
    
#     projected_img = torch.zeros_like(img)
    
#     for h in range(img_height):
#         for w in range(img_width):
#             x, y = dst_pixel_coords[:, h, w, 0], dst_pixel_coords[:, h, w, 1]
            
#             x, y = torch.round(x), torch.round(y)
#             mask = (x >= 0) * (x < img_width) * (y >= 0) * (y < img_height)
#             x, y = x[mask].type(torch.long), y[mask].type(torch.long)

#             if len(x) > 0 and len(y) > 0:
#                 projected_img[mask, :, y, x] = img[mask, :, h, w]

#     valid_points = torch.ones_like(dst_pixel_coords.abs().max(dim=-1)[0] <= 1)
#     return projected_img, valid_points

# def forward_warp_v1(img, depth, pose, intrinsics, extrinsics=None, rotation_mode='euler', padding_mode='zeros'):
#     check_sizes(img, 'img', 'B3HW')
#     check_sizes(depth, 'depth', 'BHW')
#     check_sizes(pose, 'pose', 'B6')
#     check_sizes(intrinsics, 'intrinsics', 'B33')
...
#     projected_img = torch.zeros_like(img)
#     valid_points = torch.zeros(batch_size, img_height, img_width)
...
def forward_warp(img, depth, pose, intrinsics, extrinsics=None, rotation_mode='euler', padding_mode='zeros'):
    ...
    # camera coordinates
    cam_coords = pixel2cam(depth, intrinsics.inverse().double())                     # [B,3,H,W]
    aux = torch.ones(batch_size, 1, img_height, img_width).double()
    cam_coords = torch.cat((cam_coords, aux), dim=1)                        # [B,4,H,W]
    assert cam_coords.shape == (batch_size, 4, img_height, img_width), "Camera coordinates not [B, 4, H, W]"
            ...
            for x in xs:
                for y in ys:
                    ...
                    if len(mx) > 0 and len(my) > 0:
                        ...
                        valid_points[mask, my, mx] = cond.type(torch.long) + (1 - cond.type(torch.long)) * valid_points[mask, my, mx]
                        dists[mask, my, mx] = cond.type(torch.double) * d + (1 - cond.type(torch.double)) * dists[mask, my, mx]
